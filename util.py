...
def get_sinusoid_encoding(num_tokens, token_len):
    ...
    sinusoid_table = np.array([get_position_angle_vec(i)
...
class FindFacesGeneric():
    ...
    def rescale_landmark_crop(self, landmarks: list, image, rescale_face_cropped: float):
        ...
        face_ = img[y_min - var_y:y_max + var_y,
                    x_min - var_x:x_max + var_x].copy()
    ...
    def save_face_images(self, path_to_save: str, rescale_face=1, size=(224, 224)):
        ...
        for key in tqdm(self.pointers.keys()):
            ...
            try:
                image_path = self.pointers[key][keys[0]].replace(
                    ".pts", ".jpg")
            except:
...
class PlotGrapich():
    ...
    def compare_histograms(self, histograms: list, figsize=(10, 5)):
        ...
        fig, ax = plt.subplots(figsize=figsize)
        bar = sns.barplot(data=df, x="qtd_face",
                          y="count", hue="Dataset", ax=ax)

        for container in bar.containers:
            ax.bar_label(container, fmt=lambda x: "{}".format(
                int(round(np.exp(x-1)))))
